    ...
    def propagate_next(gl, output_layer):
        ...
        for x in range(gl.layer_width - 1):
            for y in range(gl.layer_height - 1):
                ...
                gl.add_candidates(gl.layer_array[x][y], candidate_list)

                # filter candidates with rest of 3 up links
                candidate_list = gl.filter_candidates(gl.layer_array[x2][y], candidate_list, 1, 0)
                candidate_list = gl.filter_candidates(gl.layer_array[x][y2], candidate_list, 0, 1)
                matching_cell_list = gl.filter_candidates(gl.layer_array[x2][y2], candidate_list, 1, 1)

                # spread the matching cells(spatial locality)
                gl.matching_cell_spread(x, y, matching_cell_list, output_layer)
    ...
    def detect_new_cells_next(gl, label, new_cells):
        ...
        for x in range(gl.layer_width - 1):
            for y in range(gl.layer_height - 1):
                ...
                is_cell_new = True
                upper_cell = None
                for cell in gl.new_cell_array[x][y].get_up_links(0, 0):
                    if cell.get_child(1, 0) != gl.new_cell_array[x2][y]:
                        continue
                    if cell.get_child(0, 1) != gl.new_cell_array[x][y2]:
                        continue
                    if cell.get_child(1, 1) != gl.new_cell_array[x2][y2]:
                        continue
                ...
                # generate new cell
                if is_cell_new:
                    top_left = gl.new_cell_array[x][y]
                    top_right = gl.new_cell_array[x2][y]
                    bottom_left = gl.new_cell_array[x][y2]
                    bottom_right = gl.new_cell_array[x2][y2]
